document.addEventListener('DOMContentLoaded',()=>{const{defaultEncoding,translateDelay,msgToTraditionalChinese,msgToSimplifiedChinese}=GLOBAL_CONFIG.translate
...
const setLang=()=>{document.documentElement.lang=targetEncoding===1?'zh-TW':'zh-CN'}
const translateText=(txt)=>{if(!txt)return''
...
return txt}
const translateBody=(fobj)=>{const nodes=typeof fobj==='object'?fobj.childNodes:document.body.childNodes
for(const node of nodes){if(['BR','HR'].includes(node.tagName)||node===translateButtonObject)continue
if(node.nodeType===Node.ELEMENT_NODE){const{tagName,title,alt,placeholder,value,type}=node
...
translateBody(node)}else if(node.nodeType===Node.TEXT_NODE){node.data=translateText(node.data)}}}
const translatePage=()=>{if(targetEncoding===1){currentEncoding=1
targetEncoding=2
translateButtonObject.textContent=msgToTraditionalChinese
isSnackbar&&btf.snackbarShow(snackbarData.cht_to_chs)}else if(targetEncoding===2){currentEncoding=2
targetEncoding=1
...
const Traditionalized=(cc)=>{let str=''
const ss=JTPYStr()
const tt=FTPYStr()
for(let i=0;i<cc.length;i++){if(cc.charCodeAt(i)>10000&&ss.indexOf(cc.charAt(i))!==-1){str+=tt.charAt(ss.indexOf(cc.charAt(i)))}else str+=cc.charAt(i)}
return str}
const Simplized=(cc)=>{let str=''
const ss=JTPYStr()
const tt=FTPYStr()
for(let i=0;i<cc.length;i++){if(cc.charCodeAt(i)>10000&&tt.indexOf(cc.charAt(i))!==-1){str+=ss.charAt(tt.indexOf(cc.charAt(i)))}else str+=cc.charAt(i)}
return str}
const translateInitialization=()=>{if(translateButtonObject){if(currentEncoding!==targetEncoding){translateButtonObject.textContent=targetEncoding===1?msgToSimplifiedChinese:msgToTraditionalChinese
setLang()
setTimeout(translateBody,translateDelay)}}}
window.translateFn={translatePage,Traditionalized,Simplized,translateInitialization}
